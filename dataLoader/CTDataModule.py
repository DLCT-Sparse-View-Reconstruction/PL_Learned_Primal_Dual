...
class CTDataModule(LightningDataModule):
    def __init__(self, data_dir: str, ts_data_dir: str, batch_size: int):
    ...
    def setup(self, stage=None):
      ...
      # load the numpy data array
        ...
        print("DONE__Loading the dataset__")
        
        if self.ts_data_dir != None:
            print("Testing -- BEGIN__Loading the dataset__")
            with open(self.ts_data_dir, 'rb') as f:
                test_dataset = torch.Tensor(np.load(f))
            print("Testing -- DONE__Loading the dataset__")         
